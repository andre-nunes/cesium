define(['../Core/DeveloperError',
	...
	var processor = function(){
		...
		var processGxFlyToNode = function(node) {
			...
			// TEST
			// my.logDebug('Duration: ' + result.duration);
			...
			if (obj) {
				val = (function(camNode){
					...
					coords.push( parseFloat(v.longitude), parseFloat(v.latitude), parseFloat(v.altitude)*FEET2M);
			...
			// TEST
			// my.logDebug(JSON.stringify(result));
		...
		// array of animation item
		// TBD specify items

		my.logDebug = function(string) {
			if (my.debug) {
		...
		my.processTour = function(kmlNode) {
			var n, i, o, pl, item;
			my.logDebug("Reached main point");
			if (kmlNode === undefined)
				throw DeveloperError('Missing gx:Tour node');
			else if (kmlNode.nodeName !== 'gx:Tour')
				throw DeveloperError('Bad node!');
			...
			my.logDebug('Tour name: ' + _context.tourName);
			...
			for (i=0; i<pl.childNodes.length; i++) {
				...
				switch(o.nodeName) {
					case 'gx:FlyTo':
						// my.logDebug('Processing ' + o.nodeName);
					...
					case 'gx:TourControl':
						...
						break;
					default:
						// my.logDebug('Skipping unknown node ' + o.nodeName);
						break;
				}
			}
		};
